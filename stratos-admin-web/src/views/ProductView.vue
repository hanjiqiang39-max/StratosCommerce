...
const errors = reactive({ title: '', categoryId: '', price: '', stock: '' })
const form = reactive({
...
const shown = computed(() => {
  ...
  return records.value.filter((item) => String(item.title || item.spuName || '').toLowerCase().includes(key))
})

async function load() {
  const page = await productList({ pageNum: 1, pageSize: 50, status: -1, keyword: keyword.value || undefined })
  records.value = page.records || []
  tree.value = (await categoryTree()) || []
  if (!form.categoryId) form.categoryId = findFirstLeaf(tree.value)
}
...
function resetForm() {
  ...
  Object.assign(errors, { title: '', categoryId: '', price: '', stock: '' })
}
...
function validate() {
  ...
  errors.price = editing.value ? '' : numberRange(form.skuList[0].price, 0.01, 999999, '售价')
  errors.stock = editing.value ? '' : numberRange(form.skuList[0].stock, 0, 999999, '库存', true)
...
async function save() {
  ...
  await saveProduct({
    ...
    spuName: form.spuName || form.title.trim(),
  ...
  message.success(form.id ? '商品已更新' : '已保存')
  resetForm()
  await load()
}
...
async function onRowFile(event: Event) {
  ...
  if (form.id === item.id) form.mainImage = data.url
  await load()
}
...
async function remove(item: any) {
  if (!window.confirm(`确定删除「${item.title || item.spuName}」？商城将不再展示。`)) return
...
<template>
  <AdminPage title="商品" extra="发布后会出现在商城首页和分类页，无图商品可以在这里补图或改图">
    <form class="admin-card grid grid-cols-2 gap-5 p-6" @submit.prevent="save">
      ...
      <div class="col-span-2 flex gap-2">
        <button type="submit" class="h-11 rounded-full bg-sky-500 px-6 text-white">
          {{ editing ? '保存修改' : '发布商品' }}
        </button>
    ...
    <div class="admin-card">
      <div class="border-b border-slate-100 px-5 py-4">
        <input v-model="keyword" class="admin-input max-w-sm" placeholder="搜索商品标题" maxlength="40" @keyup.enter="load" />
      </div>
        ...
        <tbody>
          <tr v-for="item in shown" :key="item.id">
            ...
            <td class="space-x-2 whitespace-nowrap">
              ...
              <button class="admin-btn" @click="updateProductStatus(item.id, item.status === 1 ? 0 : 1).then(load)">
                {{ item.status === 1 ? '下架' : '上架' }}
              </button>
          ...
          <tr v-if="!shown.length">
            <td colspan="4" class="py-10 text-center text-mute">{{ records.length ? '没有匹配的商品' : '还没有商品' }}</td>
          </tr>
        </tbody>
      </table>
