...
export const CODE_RE = /^[A-Za-z0-9_-]{2,32}$/
...
export function codeValue(value: unknown, label: string) {
  const text = String(value ?? '').trim()
  if (!text) return `请填写${label}`
  return CODE_RE.test(text) ? '' : `${label} 2-32 位，仅字母数字或下划线`
}
...
export function dateOrder(start?: string, end?: string, startLabel = '开始时间', endLabel = '结束时间') {
  if (!start || !end) return `请选择${startLabel}和${endLabel}`
