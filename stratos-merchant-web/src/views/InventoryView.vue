...
import { productSkus, updateSkuStock } from '@/api'
...
const lowCount = computed(() => records.value.filter((item) => stockTone(item) !== 'ok').length)
...
const barOption = computed<EChartsOption>(() => {
  ...
  const names = rows.map((item) => chartLabel(skuLabel(item), 10))
  return {
    color: ['#0d9488', '#f59e0b'],
    tooltip: {
      trigger: 'axis',
...
async function load() {
  ...
  records.value = (await productSkus(auth.shopId)) || []
...
async function save(item: any) {
  ...
  try {
    await updateSkuStock(item.id, stock, auth.shopId!)
...
<template>
  <MerchantPage title="库存" extra="按商品名称看可售和锁定库存，库存偏低可以直接改。">
    ...
    <div class="grid grid-cols-1 gap-4 xl:grid-cols-[1.3fr_1fr]">
      <div class="admin-card p-5">
        ...
        <p class="mb-3 text-xs text-mute">按当前筛选结果展示前 8 个规格，横轴是数量</p>
      ...
      <div class="admin-card p-5">
        ...
        <p class="mb-3 text-xs text-mute">充足、预警（≤10）和售罄的规格占比</p>
      ...
      <table class="admin-table">
        ...
        <tbody>
          <tr v-for="item in shown" :key="item.id">
            ...
            <td>
              <button class="admin-btn" :disabled="actingId === item.id" @click="save(item)">保存</button>
            </td>
          </tr>
